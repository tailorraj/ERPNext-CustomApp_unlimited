...
login.bind_events = function() {
	...
	$(".form-signup").on("submit", function(event) {
		event.preventDefault();
		var args = {};
		args.cmd = "unlimited.www.login.sign_up";
		// args.cmd = "frappe.core.doctype.user.user.sign_up";
		args.email = ($("#signup_email").val() || "").trim();
		args.redirect_to = get_url_arg("redirect-to") || '';
		args.full_name = ($("#signup_fullname").val() || "").trim();
		args.dob = ($("#signup_dob").val() || "").trim();
		args.phone_number = ($("#signup_phonenumber").val() || "").trim();
		args.address = ($("#signup_address").val() || "").trim();
		args.guardian_name = ($("#signup_guardian").val() || "").trim();
	...
	$("#signup_dob").on("change", function(){
		...
		if(age < 18)
		{
			...
			$(".minor").prop('required',true);
		}
		else
		{
			$(".minor").css("display", "none");
			$(".minor").prop('required',false);
			$(".minor").val("");
		}
	});
}
		...
		200: function(data) {
			if(data.message == 'Logged In'){
				...
				if(cint(data.message[0])==0) {
					...
					// Clear signup form after success.
					$("#signup_fullname").val("")
					$("#signup_email").val("")
					$("#signup_dob").val("")
					$("#signup_guardian").val("")
